...
- Removed direct dependency on `requests` and `urllib3` from `frogspy.py` (moved to `frogspy_scraper.py`)
- `requests` session lifecycle now managed by context manager in `main()`
