...
def run_gui():
    ...
    _asset_dir   = os.environ.get("FROGSPY_ASSET_DIR", _script_dir)
    _frog_ico    = os.path.join(_script_dir, "frogspy_frog_icon.ico")  # title bar
    _bag_ico     = os.path.join(_script_dir, "frogspy_icon.ico")        # taskbar
    _frog_png    = os.path.join(_asset_dir, "frogspy_frog.png")
    _lady_png    = os.path.join(_asset_dir, "frogspy_lady.png")
    _state_file  = os.path.join(_script_dir, "frogspy_mascot.txt")

    # ── Icons: frog on title bar, bag on taskbar (Windows ctypes) ────────────
    # iconbitmap sets the title-bar icon; ctypes WM_SETICON overrides the
    # taskbar icon independently — Windows-only, silently skipped elsewhere.
    def _set_icons():
        try:
            if os.path.exists(_frog_ico):
                root.iconbitmap(_frog_ico)
        except Exception:
            pass
        try:
            import sys, ctypes
            if sys.platform == "win32" and os.path.exists(_bag_ico):
                ICON_BIG   = 1
                WM_SETICON = 0x0080
                hWnd = ctypes.windll.user32.GetParent(root.winfo_id())
                hIcon = ctypes.windll.user32.LoadImageW(
                    None, _bag_ico, 1,  # IMAGE_ICON
    ...
    # ── Clickable mascot (top-right of header) ──────────────────────────────
    # Default: frog. Click once → lady (easter egg). Persists via state file.
    # Click again → back to frog. State survives restarts.
    try:
        from PIL import Image as _PILImage, ImageTk as _ImageTk

        _PANEL_RGBA = (22, 33, 62, 255)  # matches PANEL = "#16213e"

        def _load_mascot_img(path):
            """Open PNG, composite onto PANEL bg, return ImageTk.PhotoImage."""
            pil = _PILImage.open(path).convert("RGBA")
            bg  = _PILImage.new("RGBA", pil.size, _PANEL_RGBA)
            out = _PILImage.alpha_composite(bg, pil).convert("RGB")
            return _ImageTk.PhotoImage(out)

        # Read persisted state
        def _read_mascot_state():
            try:
        ...
        _mascot_state = [_read_mascot_state()]  # mutable cell

        # Pre-load both images (so click is instant)
        _img_frog = _load_mascot_img(_frog_png) if os.path.exists(_frog_png) else None
        _img_lady = _load_mascot_img(_lady_png) if os.path.exists(_lady_png) else None

        if _img_frog or _img_lady:
            _mascot_lbl = tk.Label(top, bg=PANEL, bd=0, highlightthickness=0,
                                   cursor="hand2")
            _mascot_lbl.pack(side="right", padx=18, pady=4)

            def _update_mascot_display():
                img = _img_lady if _mascot_state[0] == "lady" else _img_frog
                if img:
                    _mascot_lbl.config(image=img)
                    _mascot_lbl.image = img  # keep reference alive

            def _on_mascot_click(_event=None):
                # Toggle state
                _mascot_state[0] = "lady" if _mascot_state[0] == "frog" else "frog"
                _write_mascot_state(_mascot_state[0])
                _update_mascot_display()

            _mascot_lbl.bind("<Button-1>", _on_mascot_click)
            _update_mascot_display()  # show correct image on startup

    except Exception:
        pass  # mascot is purely cosmetic — never crash on failure
