...
def main():
    parser = argparse.ArgumentParser(description="Bazaar Price Checker using FrogTracker")
    ...
    parser.add_argument("--output", default="bazaar_check_output.txt", help="Output file")
    args = parser.parse_args()

    session = requests.Session()
    start_time = datetime.datetime.now()
    print(f"\nBazaar Price Check starting at {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    summary = (
        f"\n{'='*60}\n"
        f"SUMMARY for {args.trader} -- {end_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"{'='*60}\n"
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        f.write(f"Bazaar Price Check -- {args.trader} -- {start_time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
